...
parser.add_argument('--root_path', type=str,
                    default='../data/ACDC-RLV', help='Name of Experiment')
parser.add_argument('--exp', type=str,
                    default='ACDC_RLV/Uncertainty_Rectified_Pyramid_Consistency', help='experiment_name')
parser.add_argument('--model', type=str,
                     default='unet_urpc_adapter', help='model_name')
...
def save_loss_curves(allloss, save_path, interval=5):
    total_loss = allloss['total_loss']
    loss_ce = allloss['loss_ce']
    loss_dice = allloss['loss_dice']

    # 选择指定间隔的数据点
    selected_indices = range(0, len(total_loss), interval)
    selected_total_loss = [total_loss[i] for i in selected_indices]
    selected_loss_ce = [loss_ce[i] for i in selected_indices]
    selected_loss_dice = [loss_dice[i] for i in selected_indices]

    epochs = np.arange(0, len(total_loss), interval)

    # 平滑曲线
    epochs_smooth = np.linspace(epochs.min(), epochs.max(), 300)
    spl_total_loss = make_interp_spline(epochs, selected_total_loss, k=3)
    spl_loss_ce = make_interp_spline(epochs, selected_loss_ce, k=3)
    spl_loss_dice = make_interp_spline(epochs, selected_loss_dice, k=3)

    total_loss_smooth = spl_total_loss(epochs_smooth)
    loss_ce_smooth = spl_loss_ce(epochs_smooth)
    loss_dice_smooth = spl_loss_dice(epochs_smooth)

    plt.figure(figsize=(10, 6))
    plt.plot(epochs_smooth, total_loss_smooth, label='Total Loss')
    plt.plot(epochs_smooth, loss_ce_smooth, label='CE Loss')
    plt.plot(epochs_smooth, loss_dice_smooth, label='Dice Loss')
    ...
    # 将图表保存为.tiff格式
    plt.gca().xaxis.set_major_locator(MaxNLocator(integer=True))  # 强制 x 轴刻度为整数
    plt.savefig(save_path + '/loss_curves.tiff')
    plt.close()

def plot_loss_curves_for_epochs(allloss, save_path, start_epoch, end_epoch, interval):
    total_loss = allloss['total_loss']
    loss_ce = allloss['loss_ce']
    loss_dice = allloss['loss_dice']

    epochs = range(len(total_loss))
    selected_epochs = epochs[start_epoch:end_epoch]

    # 选择指定间隔的数据点
    selected_indices = range(start_epoch, end_epoch, interval)
    selected_total_loss = [total_loss[i] for i in selected_indices]
    selected_loss_ce = [loss_ce[i] for i in selected_indices]
    selected_loss_dice = [loss_dice[i] for i in selected_indices]

    # 平滑曲线
    epochs_smooth = np.linspace(start_epoch, end_epoch, 300)
    spl_total_loss = make_interp_spline(selected_epochs, selected_total_loss, k=3)
    spl_loss_ce = make_interp_spline(selected_epochs, selected_loss_ce, k=3)
    spl_loss_dice = make_interp_spline(selected_epochs, selected_loss_dice, k=3)
    ...
    plt.xlabel('epoch')
    plt.ylabel('Loss')
    plt.title('Training Loss Curves（epoch：{}-{}）'.format(start_epoch, end_epoch))
    plt.legend()
    plt.grid(True)

    # 将图表保存为.tiff格式
    plt.gca().xaxis.set_major_locator(MaxNLocator(integer=True))  # 强制 x 轴刻度为整数
    plt.savefig(save_path + '/up_result.tiff')
    plt.close()
...
def train(args, snapshot_path):
    ...
    model = UNet_URPC_Adapter(in_chns=1, class_num=num_classes).cuda()
    ...
    print("Total slices: {}, labeled slices: {}".format(
    ...
    allloss = {'total_loss': [], 'loss_ce': [], 'loss_dice': []}
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            lr_ = base_lr * (1.0 - iter_num / max_iterations) ** 0.0009
            ...
            allloss['total_loss'].append(loss.item())
            allloss['loss_ce'].append(loss_ce.item())
            allloss['loss_dice'].append(loss_dice.item())
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                ...
                for class_i in range(num_classes-1):
                    ...
                    writer.add_scalar('info/val_{}_f1_score'.format(class_i + 1),
                ...
                mean_f1_score = np.mean(metric_list, axis=0)[4]
                ...
                writer.add_scalar('info/val_mean_f1score', mean_f1_score, iter_num)
                ...
                logging.info(
                    'iteration %d : mean_dice : %f mean_hd95 : %f mean_precision : %f mean_recall : %f mean_f1_score : %f' % (iter_num, performance, mean_hd95, mean_precision, mean_recall, mean_f1_score))
                model.train()
    ...
    save_loss_curves(allloss, '../result/loss')
    plot_loss_curves_for_epochs(allloss, '../result/loss', start_epoch=10000, end_epoch=15000, interval=5)
...
if __name__ == "__main__":
    ...
    print("chz test path 2: ", snapshot_path)
